...
- Voice loop (voice in → task → voice out): `--auto-send` flips `autoSendAfterSpeech` (send the transcript without tapping), and reply read-out works on glasses via the Rokid assistserver TTS fallback — `tts/RokidAssistTtsEngine.kt` binds the vendor AIDL (`com.rokid.os.sprite.tts.TtsService`, explicit ComponentName; protocol reverse-engineered, `playTtsMsg(text, tag, listener)`). System TTS engines stay preferred on phones; Rokid is fallback-only.
